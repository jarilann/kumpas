  ...
  class _LessonContentScreenState extends State<LessonContentScreen> {
    ...
    Future<void> _initializeVideo() async {
      ...
      if (sign.videoAssetPath != null && sign.videoAssetPath!.isNotEmpty) {
        try {
          // Try to get download URL from Firebase Storage if it's a storage
  path
          final videoUrl = await _getVideoUrl(sign.videoAssetPath!);

          // Use the new networkUrl constructor (replaces deprecated
  .network)
          _videoController =
  VideoPlayerController.networkUrl(Uri.parse(videoUrl))
          ...
          try {
            _videoController =
  VideoPlayerController.asset(sign.videoAssetPath!)
              ..initialize().then((_) {
                setState(() {
                  _videoInitialized = true;
                  _isVideoAvailable = true;
    ...
    Future<String> _getVideoUrl(String path) async {
      // If path looks like a Firebase Storage path (starts with videos/ or
  similar)
    ...
    Future<void> _checkExistingProgress() async {
      ...
      final states = await
  ProgressService.instance.getLessonStates(kModules);
      ...
      final viewedAll = allSignIds.isNotEmpty &&
  allSignIds.every(viewedIds.contains);
    ...
    void _goToQuiz() {
      Navigator.of(context).pushReplacement(
        MaterialPageRoute(
          builder: (_) => QuizScreen(module: widget.module, lesson:
  widget.lesson),
        ),
      );
    }
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                ...
                textAlign: TextAlign.center,
                style: TextStyle(color: AppColors.textWhiteMuted, fontSize:
  14),
              ),
